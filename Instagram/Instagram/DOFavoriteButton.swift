...
public class DOFavoriteButton: UIButton {
    ...
    @IBInspectable public var imageHeight: Double = 16.0 {
        didSet {
            createLayers(image: image);
        }
    }
    @IBInspectable public var imageWidth: Double = 16.0 {
        didSet {
            createLayers(image: image);
        }
    }
    ...
    private func createLayers(image image: UIImage!) {
        ...
        let imageFrame = CGRect(x: frame.size.width / 2 - 16, y: frame.size.height / 2 - 16, width: CGFloat(imageWidth), height: CGFloat(imageHeight));
